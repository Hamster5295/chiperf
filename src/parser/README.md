...
| §4 词法 | 字符串 → 数值 → 裸词；四种进制 + Verilog 字面量 + 4 态；`x`/`z` 裸词规则 |
| §5 语法 | 完整 EBNF；两条上下文规则（`at=` 的值、`msg` 的载荷） |
| §6 时间模型 | 单一全局时钟 `clock` 的 `(cycle, phase)`；`at=` 单条覆盖；clk-free 模式 |
| §7 事件 | 7 种记录 + `at=`/`async=`/`note=` 属性 + 未知属性忽略 |
| §8 指令 | `@meta` / `@end`；未知指令忽略 |
| §9 派生量 | 计数器、数值、状态机、在飞条目（= 该级持有同一个值的区间）、占用度、气泡、驻留、区间差 |
